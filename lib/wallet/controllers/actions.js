...
const bulkService = new BulkService(valueObserver);
const transactionService = new TransactionService();

valueObserver.startObserve();
const actions = {},
...
actions.setGasBoost = async function (req, res, next) {
  valueObserver.toggleGasBoostMode();

  const values = req.body.values;
  
  if (values != undefined && values.lengh != 0) {
    valueObserver.updateGasBoostTable(values);
  }

  const response = { status: "ok", boostStatus:valueObserver.boostStatus() };
  res.status(200).json(response);
};
...
actions.lockWallet = async function (req, res, next) {
  ...
  if (balance != "0" && !isLocked) {
    await adminService.lockWallet(ownerAcnt, addr);
    response.status = "LockSuccuess";
  } else {
    response.status = "Already locked";
  }
...
actions.getTransferHistroy = async function (req, res, next) {
  const addr = req.body.addr;
  try {
    const transferDatas = await transactionService.getTransferHistroy(addr);
    const response = { status: "ok", result:transferDatas };
    res.status(200).json(response);
  } catch (error) {
    const response = { status: "failed", result:[] };
...
actions.transferFeeAsync = async function (req, res, next) {
  ...
  /// Temp
  bulkService
    .emitSendFeeByAdminBulk(ownerAccount, account, contractAddr)
    .catch((error) => {
...
actions.approveAsync = async function (req, res, next) {
  ...
  /// Temp
  bulkService
    .emitApproveByAdminBuk(ownerAccount, account, contractAddr)
    .catch((error) => {
      const response = { status: "failed" };
      res.status(500).json(response);
      return;
...
actions.getTransferFee = async function (req, res, next) {
  ...
  const feeValue = await adminService.getDisplayiedFeeAmount(
    fromAddr,
    toAddr,
    amount,
    fee
