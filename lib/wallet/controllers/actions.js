...
actions.transferAdmin = async function(req, res, next) {
    ...
    const duplicateValue = await conn.query("SELECT count(*) FROM batchTransfer WHERE name = ? and phoneNum = ? and sendStatus = ?", [dupQurey.name, dupQurey.phoneNum, 'SendSuccess']);

    if (duplicateValue[0]['count(*)'] > 0) {
        const response = { "status": "duplicate", "user": { name: nameVal, phoneNum: phoneNumVal } };
    ...
    try {
        ...
        // let gasPrice = web3.utils.toBN(web3.eth.getGasPrice());
        // let gasPriceMore = gasPrice.mul(web3.utils.toBN(1.1));

        estimatedGas = estimatedGas * 2 < 5000000 ? estimatedGas * 2 : 5000000;

        // console.log("Estimate Gas Price : " + gasPriceMore.toNumber());
        console.log("Estimate Gas Value : " + estimatedGas);
        ...
        if (result.status) {
            const values = { wallet_address: toAddr, balance: balanceValue.toNumber(), name: nameVal, phoneNum: phoneNumVal };
            const duplicateValue = await conn.query("INSERT INTO batchTransfer(wallet_address, balance, name, phoneNum, email, userToken, txHash, sendStatus) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", [values.wallet_address, values.balance, values.name, values.phoneNum, '', 'admin', result.transactionHash, "SendSuccess"]);

            const response = { "result": true, "status": "Success", "data": result, "balance": balanceValue.toNumber(), "user": { name: nameVal, phoneNum: phoneNumVal }, "txHash": result.transactionHash };
            response.req = req.body;
            res.status(200).json(response);
            console.log("Done : " + values);
        } else {
            const values = { wallet_address: toAddr, balance: balanceValue.toNumber(), name: nameVal, phoneNum: phoneNumVal };
            const duplicateValue = await conn.query("INSERT INTO batchTransfer(wallet_address, balance, name, phoneNum, email, userToken, txHash, sendStatus) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", [values.wallet_address, values.balance, values.name, values.phoneNum, '', 'admin', result.transactionHash, "SendFail"]);

            const response = { "result": false, "status": "TxError", "data": result, "user": { name: nameVal, phoneNum: phoneNumVal } };
            response.req = req.body;
            res.status(200).json(response);
        ...
        console.log("Error");
        console.log(error);
        const values = { wallet_address: toAddr, name: nameVal, phoneNum: phoneNumVal };
        const duplicateValue = await conn.query("INSERT INTO batchTransfer(wallet_address, balance, name, phoneNum, email, userToken, txHash, sendStatus) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", [values.wallet_address, values.balance, values.name, values.phoneNum, '', 'admin', '', "Unknown"]);

        const response = { "result": false, "status": "UnknownError", "user": { name: nameVal, phoneNum: phoneNumVal } };
        response.req = req.body;
        res.status(200).json(response);
    }
