...
actions.approve = async function(req, res, next) {
    ...
    try {
        ...
        const encodeABI = await srtContract.methods
            .approve("0xb29082d1E5e2F5ec3D7480200395ac867948E469", approveMaxValue)
            .encodeABI();
...
actions.transferAdmin = async function(req, res, next) {
    ...
    try {
        ...
        const nonceVal = await web3.eth.getTransactionCount(account.address, 'latest'); // nonce starts counting from 0
        ...
        if (result.status) {
            const values = {
                ...
                balance: balanceValue.toNumber(),
                name: nameVal,
                phoneNum: phoneNumVal,
            };
            const duplicateValue = await conn.query(
                "INSERT INTO batchTransfer(wallet_address, balance, name, phoneNum, email, userToken, txHash, sendStatus) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", [
