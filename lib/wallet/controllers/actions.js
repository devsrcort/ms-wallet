...
actions.transferAdmin = async function(req, res, next) {
    ...
    try {
        ...
        if (result.status) {
            ...
            const response = { "result": true, "data": result, "balance": balanceValue.toNumber(), "txHash": result.transactionHash };
            response.req = req.body;
            res.status(200).json(response);
        } else {
            const response = { "result": false, "data": result, "user": { name: nameVal, phoneNum: phoneNumVal, email: emailVal } };
            response.req = req.body;
            res.status(200).json(response);
        }
