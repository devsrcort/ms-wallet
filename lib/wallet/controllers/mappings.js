...
const trasferValidator = spieler([
    check('toAddr').exists().withMessage('address met be provied')
    .trim(),

    check('amount').exists().withMessage('amount met be provied')
    .trim(),

    check('pk').exists().withMessage('to_addr met be provied')
    .trim()
]);
...
const transferFromValidator = spieler([
    ...
    check('pk').exists().withMessage('to_addr met be provied')
    .trim(),
    check('fromAddr').exists().withMessage('to_addr met be provied')
    .trim()
]);
