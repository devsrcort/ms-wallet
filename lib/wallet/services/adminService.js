...
class AdminService {
    ...
    async getTransferWithFeeAbi(fromAddr, toAddr, amount, fee) {
        ...
        log.info(`Converted Value : ${convertAmount} : ${convertFee}`);
    ...
    async transferTokenByClient(fromAddr, toAddr, amount, fee) {
        if (!(this.web3.utils.isAddress(fromAddr) || !this.web3.utils.isAddress(toAddr) || !Number.isInteger(amount))) {
        ...
        return this.txEmitter.push(async(fromAddr) => {
            this.executeTx(operator, encodedAbi, gasPrice, this.ctrlAddress).then(async function() {
                const userApiUrl = process.env.NODE_ENV == "production" ?
                    "https://app.srt-wallet.io" :
                    "https://app.dev.srt-wallet.io";

                log.info(`userApiUrl : ${userApiUrl}`);
                await axios.post(userApiUrl + "/users/commitAmount", {
                    addr: fromAddr,
                    adminPw: process.env.ADMINKEYRAW
                });
                log.info(`commitAmount Done`);

                await axios.post(userApiUrl + "/users/commitBalance", {
                    addr: fromAddr,
                    adminPw: process.env.ADMINKEYRAW
                });

                log.info(`commitBalance Done`);
        ...
        return new Promise((resolve, reject) => {
            op.attempt(async(currentAttempt) => {
                ...
                const calcGasPrice = this.web3.utils.toWei(
                    String(Math.floor(50 * estimatedGas)),
                    "gwei"
                );
                const transferFee = this.web3.utils.toWei(
                    String(Math.floor(parseFloat(50))),
                    "gwei"
                );
