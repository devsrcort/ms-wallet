...
class AdminService {
  ...
  async transferTokenByClient(fromAddr, toAddr, amount, fee) {
    ...
    return this.executeTx(operator, encodedAbi, gasPrice, this.ctrlAddress);
        ...
        async function (currentAttempt) {
          ...
          await this.web3.eth
            ...
            .catch((error) => {
              ...
              if (error.receipt != undefined && error.receipt.status == false) {
  ...
  async genGeneralSignTx(fromAccount, encodeABI, gasPrice, contractAddress) {
    const estimateGasPrice =
      this.valueObserver.getGasPrice("FastGasPrice") < 10
        ? 15
        : this.valueObserver.getGasPrice("FastGasPrice");
